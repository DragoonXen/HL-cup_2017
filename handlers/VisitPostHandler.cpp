...
#include "VisitPostHandler.h"

void ::VisitPostHandler::process(Buffer *buffer) {
    ...
    if (buf[13] == 110) { //neW
        ...
        visit->updateVisitOutput(buffer);
    } else {
        ...
        ::storage::users->visits.push_back(visit);
        ::storage::locations->visits.push_back(visit);
        visit->updateVisitOutput(buffer);
    }
}
